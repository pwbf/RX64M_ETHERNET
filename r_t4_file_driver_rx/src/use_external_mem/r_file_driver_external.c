...
int32_t file_open_sub(uint8_t *file_path, uint8_t mode_flag)
{
    ...
    {
        open_mode = (TFAT_FA_WRITE | TFAT_FA_CREATE_ALWAYS);
    }
    else
    {
        open_mode = (TFAT_FA_READ);
    }
    ...
    frslt = R_tfat_f_open(&fp[index], (const char *)p_file_path, open_mode);
    if (frslt != TFAT_FR_OK)
    {
        id[index] = 0;
        return -1;
    ...
    R_tfat_f_stat((const char *)p_file_path, &finfo[index]);
...
int32_t file_read_sub(int32_t file_id, uint8_t *buf, int32_t read_size)
{
    ...
    frslt = R_tfat_f_read(&fp[file_id], buf, (uint16_t)read_size, &read_byte);
    if (frslt != TFAT_FR_OK)
    {
        return -1;
    }
...
int32_t file_write_sub(int32_t file_id, uint8_t *buf, int32_t write_size)
{
    ...
    FRESULT frslt;
    uint16_t write_byte;
    frslt = R_tfat_f_write(&fp[file_id], buf, (uint16_t)write_size, &write_byte);
    if (frslt != TFAT_FR_OK)
    {
        return -1;
    }
...
int32_t file_delete_sub(uint8_t *file_path)
{
    ...
    frslt  = R_tfat_f_stat((const char *)p_file_path, &tmpfinfo);
    frslt += R_tfat_f_chmod((const char *)p_file_path, 0, TFAT_AM_RDO | TFAT_AM_ARC | TFAT_AM_SYS | TFAT_AM_HID);
    frslt += R_tfat_f_unlink((const char *)p_file_path);
    if (frslt != TFAT_FR_OK)
    {
        frslt += R_tfat_f_chmod((const char *)p_file_path, tmpfinfo.fattrib, TFAT_AM_RDO | TFAT_AM_ARC | TFAT_AM_SYS | TFAT_AM_HID);
        return -1;
    }
    return 0;
}
...
int32_t make_dir_sub(uint8_t *dir_path)
{
    ...
    frslt = R_tfat_f_mkdir((const char *)p_dir_path);
    switch (frslt)
    {
        case TFAT_FR_OK:
            ret = 0;
...
int32_t remove_dir_sub(uint8_t *dir_path)
{
    ...
    frslt = R_tfat_f_unlink((const char *)p_dir_path);
    if (frslt != TFAT_FR_OK)
    {
        return -1;
    }
...
int32_t change_dir_sub(uint8_t *dir_path)
{
    ...
    frslt = R_tfat_f_opendir(&dir, (const char *)p_dir_path);
    if (frslt != TFAT_FR_OK)
    {
        return -1;
    }
...
int32_t file_rename_sub(uint8_t *old_name, uint8_t *new_name)
{
    ...
    frslt  = R_tfat_f_rename((const char *)p_old_name, (const char *)p_new_name);
    if (frslt != TFAT_FR_OK)
    {
        return -1;
    }
...
int32_t get_file_list_info_sub(uint8_t *dir_path, FILE_LIST *file_list, uint32_t num_file_list, int32_t read_index)
{
    ...
    frsrt = R_tfat_f_opendir(&dir, (const char *)tmp_path);
    if (frsrt != TFAT_FR_OK)
    {
        return -1;
    }
    ...
    {
        frsrt = R_tfat_f_readdir(&dir, &file_info);
        if (frsrt != TFAT_FR_OK)
        {
            return -1;
        }
    ...
    while (1)
    {
        ...
        if (file_info.fname[0] != 0)
        {
            ...
            if ((file_info.fattrib & TFAT_AM_RDO) == TFAT_AM_RDO)
            {
                /* ReadOnly */
                file_list->file_attr |= FILE_ATTR_RDO;
            }
            if ((file_info.fattrib & TFAT_AM_HID) == TFAT_AM_HID)
            {
                /* Hidden */
                file_list->file_attr |= FILE_ATTR_HID;
            }
            if ((file_info.fattrib & TFAT_AM_SYS) == TFAT_AM_SYS)
            {
                /* System */
                file_list->file_attr |= FILE_ATTR_SYS;
            }
            if ((file_info.fattrib & TFAT_AM_VOL) == TFAT_AM_VOL)
            {
                /* Volume Label */
                file_list->file_attr |= FILE_ATTR_VOL;
            }
            if ((file_info.fattrib & TFAT_AM_DIR) == TFAT_AM_DIR)
            {
                /* Directory */
                file_list->file_attr |= FILE_ATTR_DIR;
            }
            if ((file_info.fattrib & TFAT_AM_ARC) == TFAT_AM_ARC)
            {
                /* Arcive */
