...
//        }
        ...
        /* memory over */
        if ( store_mem_current_pos >= SERVER_CONTENTS_STORE_MEM_SIZE )
        {
            return -1;
        }
...
static void delete_contents( uint32_t contents_num )
{
    ...
    uint8_t* delete_addr = 0;

    /* memory area delete & update */
    if ( (SERVER_CONTENTS_STORE_HEAD_ADR <= contents[contents_num].file_address)
            && (contents[contents_num].file_address < SERVER_CONTENTS_STORE_END_ADR) )
    {
        /* memory area delete and  close up */
        delete_size = contents[contents_num].file_size;
        delete_addr = contents[contents_num].file_address;
        memcpy( contents[contents_num].file_address,
                (contents[contents_num].file_address + delete_size),
                ((SERVER_CONTENTS_STORE_HEAD_ADR + store_mem_current_pos) - (contents[contents_num].file_address + delete_size)) );
        store_mem_current_pos -= delete_size;

        /* file address Infomation modify */
        for (i = 0; i < now_contents_num; i++)
        {
            if ( (delete_addr < contents[i].file_address)
